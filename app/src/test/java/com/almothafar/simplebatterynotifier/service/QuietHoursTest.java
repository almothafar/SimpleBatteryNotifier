...
/**
 * Unit tests for the pure quiet-hours / silent-mode policy in {@link QuietHours}: the time-range check,
 * the critical-override gating and the corrupt-bound fallback.
...
public class QuietHoursTest {
	...
	/**
	 * {@link QuietHours#isWithinTimeRange(int, int, int)} across daytime, overnight-wrap and
	...
	public static class TimeRange {
		...
		public void matchesExpected() {
			assertEquals(label, expected, QuietHours.isWithinTimeRange(now, start, end));
		}
	}

	/**
	 * {@link QuietHours#alertsAllowedNow(boolean, boolean, boolean)} — quiet-hours gating with
	...
	public static class AlertsAllowedNow {
		...
		public void matchesExpected() {
			assertEquals(expected,
					QuietHours.alertsAllowedNow(withinWindow, isCritical, criticalIgnoresQuietHours));
		}
	}

	/**
	 * {@link QuietHours#boundOrDefaultMinutes(String, String)} — a corrupt stored quiet-hours bound
	 * falls back to that bound's default instead of crashing the alert path (issue #154). Runs under
	 * Robolectric because the fallback is logged.
	 */
	...
	public static class BoundOrDefaultMinutes {
		...
		public void validStoredBoundWins() {
			assertEquals(22 * 60 + 15, QuietHours.boundOrDefaultMinutes("22:15", DEFAULT_START));
		}

		@Test
		public void malformedStoredBoundFallsBackToDefault() {
			assertEquals(6 * 60 + 30, QuietHours.boundOrDefaultMinutes("ab:cd", DEFAULT_START));
			assertEquals(6 * 60 + 30, QuietHours.boundOrDefaultMinutes(null, DEFAULT_START));
			assertEquals(6 * 60 + 30, QuietHours.boundOrDefaultMinutes("25:99", DEFAULT_START));
		}
		...
		public void unparseableDefaultFloorsAtMidnightInsteadOfPropagating() {
			...
			assertEquals(0, QuietHours.boundOrDefaultMinutes("bad", "also bad"));
		}
	}
}
